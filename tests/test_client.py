...
async def test_firmware_update_missing_new_version(client: QnapClient) -> None:
    ...
    with aioresponses() as m:
        m.post("http://192.168.1.100:8080/cgi-bin/authLogin.cgi", body=LOGIN_SUCCESS_XML)
        # system info for current version
        m.get(
            "http://192.168.1.100:8080/cgi-bin/management/manaRequest.cgi",
            body="""<QDocRoot><authPassed>1</authPassed><func><ownContent>
                <system><firmware><version>5.1.0.2466</version></firmware></system>
            </ownContent></func></QDocRoot>""",
        )
        # firmware check with missing newVersion
        m.get(
